...
- `/import` Excelインポート（owner / 許可されたadmin）
  ...
  - 操作性: 完全一致1名・在籍・時給同一・同名重複なし・ルール矛盾なしの行は
    「自動確定済み」となり操作不要（手動変更は可能）。一括操作
    （完全一致のみ紐付け / 候補なしのみ新規登録［確認画面+件数警告付き］/
    表示中・全件の除外 / 選択解除）と9種の絞り込み、未選択行への移動導線つき
  ...
  - 店舗・月・ファイルの3点選択必須 → 照合確認画面（旧版の3種確認フロー:
    時給変更候補 / 同名キャスト候補 / 退店・在籍状態確認 を維持）
